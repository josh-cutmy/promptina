...
CREATE POLICY "Users can view all profiles for sharing"
  ...
  USING (true); -- Allow all authenticated users to see profiles for @ mentions
...
-- 9. Create view for easier querying of shared items with details
CREATE VIEW public.shared_items_with_details AS
SELECT 
  si.*,
  i.title,
  i.content,
...
JOIN public.user_profiles owner ON si.shared_by = owner.id
JOIN public.user_profiles recipient ON si.shared_with = recipient.id
